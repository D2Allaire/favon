export default {
   ...
   */
  default: /^(?:(?:\[[^\]]+\]|\([^)]+\)|\{[^}]+\})(?:[\s_.]*))*((?:[^\s_.-]+[\s_.-])*)(?:\b|[\s_.-]*)+(?:(?:[[(]?\d{4}[)\]]?)(?:(?:\b|[\s_.-]*))+)?(?:S(?:eason ?)?(\d{1,3})[\s_.\-x]*)?(?:(?:Ep?(?:isode[ ._])?[ _.]?)?(\d{1,3}(?:-\d{1,3})?(?:\.\d)?)(?:[_ ]?v\d)?[\s_.-]*)(?![^([{]*\b\d{1,3}(?:[_\s]?v\d+)?\b)(?:\w+[\s_.-]*)*?(?:(?:\[[^\]]+\]|\([^)]+\)|\{[^}]+\})(?:[\s_]*))*(?:[[({][\da-f]{8}[\])}])?(?:[^\\/]*$)/i,

  /**
   * Fallback, try to match title, season and episode from previously cleaned up string.
   */
  fallback: /^((?:[^\s-]+\b|\s)*)(?:\b|[\s-]*)+(?:S(?:eason ?)?(\d{1,3})[\s_.\-x]*)?(?:\b|[\s_-])*(?:(?:Ep?(?:isode[ ._])?[ _.]?)?(\d{1,3}(?:-\d{1,3})?(?:\.\d)?)(?:[_ ]?v\d)?)/i,
